...
const fields = ['id', 'name', 'category', 'brand', 'presentation', 'description', 'features', 'manufacturerCode', 'featured', 'sortOrder', 'storeCodes', 'cakeClass', 'saleUnit', 'bogo'];
export function applyCommercialCorrections(products, corrections) {
  ...
  for (const correction of corrections) {
    ...
    for (const [key, value] of Object.entries(correction.fields)) {
      if (!['category', 'storeCodes', 'cakeClass', 'saleUnit', 'bogo'].includes(key)) throw new Error(`Invalid commercial field: ${key}`);
      product[key] = value;
    }
  }
  return corrected;
...
export function run({ check = false } = {}) {
  ...
  const products = applyCommercialCorrections(applyCommercialCorrections(applyCopyCorrections(applyManualCorrections(master, corrections), copyCorrections), commerce), cakeResearch);
  ...
  for (const path of ['src/data/promotions.json', 'src/data/pdfThumbnails.json', 'src/data/brands.json', 'src/shared/myList.js', 'src/shared/myList.d.ts', 'src/shared/listPdf.js', 'src/shared/listPdf.d.ts']) {
  ...
  const routes = ['/', '/products', ...products.map((p) => `/products/${p.slug}`), '/mobile-app', '/terms-and-conditions', '/app-privacy', '/app-support'];
