...
test('cake classification and Silent Treatment correction survive sync; all approved product bytes persist', () => {
  ...
  assert.deepEqual(applyCommercialCorrections(applyCommercialCorrections(stale, decisions), read('../docs/catalog/refinement-2026-09-25/cake-research.json')), products);
  ...
  for (const [path, hash] of Object.entries(baseline.productHashes)) assert.equal(createHash('sha256').update(readFileSync(new URL(`../public${path}`, import.meta.url))).digest('hex'), hash);
