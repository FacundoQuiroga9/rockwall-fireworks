...
export default function BogoBadge({ product }) {
  ...
  return <span className={`bogo-ticket ${state.active ? 'bogo-ticket-active' : ''}`} aria-label={`BOGO: Buy One, Get One. ${state.active ? 'Verified active offer. Confirm application in store.' : 'Identified as BOGO in our inventory. Offer dates and conditions need store confirmation.'}`}>
