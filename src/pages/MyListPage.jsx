...
import { addPromotion, assessList, canPair, dissolveGroup, isBogoCandidate, promotionIsCurrent, removeGroup, reviewGroup, setBogoPartner, setPairCount, setPromotionItem, setQuantity, startBogo } from '../shared/myList';
...
function Quantity({ value, onChange, label }) {
  ...
  return <label className="list-quantity">{label}<input type="number" min="1" max="999" step="1" inputMode="numeric" value={draft} onChange={(e) => { setDraft(e.target.value); const n = Number(e.target.value); if (Number.isInteger(n) && n >= 1 && n <= 999) onChange(n); }} onBlur={() => setDraft(String(value))} /></label>;
}
function PromotionPicker({ update }) {
  const active = promotions.filter((p) => promotionIsCurrent(p) && ['choice', 'fixed'].includes(p.kind));
  const [id, setId] = useState('');
  const [chosen, setChosen] = useState([]);
  const promotion = active.find((p) => p.id === id);
  return <section className="list-promotion-info"><h2>Store promotions</h2>{!active.length ? <p>No current coupon campaign has been verified. The 2025 anniversary offers are expired and cannot be applied to this list.</p> : <>
    ...
    {promotion && <><p>{promotion.conditions.join(' ')}</p>{promotion.kind === 'choice' && Array.from({ length: promotion.requiredQuantity }, (_, i) => <label key={i}>Selection {i + 1}<select value={chosen[i] || ''} onChange={(e) => setChosen((old) => { const next = [...old]; next[i] = e.target.value; return next; })}><option value="">Choose product</option>{products.filter((p) => promotion.eligibleIds.includes(p.id)).map((p) => <option key={p.id} value={p.id}>{p.name} · {p.brand}</option>)}</select></label>)}<button className="list-button" type="button" disabled={promotion.kind === 'choice' && Array.from({length: promotion.requiredQuantity}, (_, i) => chosen[i]).some((value) => !value)} onClick={() => { update((list) => addPromotion(list, promotion, products, chosen.map((productId) => ({ productId, quantity: 1 })))); setId(''); setChosen([]); }}>Add configured promotion</button></>}
  </>}<p>BOGO means Buy One, Get One. Prepare an explicit pair from a marked product below. Both selections must be eligible and in the same category; brands may differ.</p></section>;
}
      ...
      {assessment.groups.map((group, index) => <section className={`list-group list-group-${group.kind}`} key={group.id} aria-label={`${group.kind === 'bogo' ? 'BOGO pair' : 'Selection'} ${index + 1}`}>
        ...
        {group.items.map((item, i) => {
          ...
          return <div className="list-product" key={`${item.productId}-${i}`}>
            {current && !item.changed && <img src={current.image} alt="" width="96" height="96" />}
            <div className="list-product-copy"><p>{shown.category} · {shown.brand || 'Brand not specified'}</p><h3>{current ? <Link to={`/products/${current.slug}`}>{shown.name}</Link> : shown.name}</h3>{shown.presentation && <p>{shown.presentation}</p>}<small>{shown.storeCodes?.filter((c) => c.sku).map((c) => `SKU ${c.sku}`).join(' · ') || (shown.manufacturerCode ? `Model ${shown.manufacturerCode}` : 'Store code not verified')}</small>
              {item.changed && <p className="list-warning">Current catalog: {current.name} · {current.category} · {current.brand} · {current.presentation}. Review the product before accepting this change.</p>}
            </div>
            <div className="list-unit-summary"><span>Paid <strong>{item.paid ?? '—'}</strong></span><span>Free <strong>{item.free ?? '—'}</strong></span><span>Total <strong>{item.quantity}</strong></span></div>
            {(group.kind === 'individual' || (group.promotion?.kind === 'choice' && promotionIsCurrent(group.promotion))) && <Quantity label={`Quantity for ${shown.name}`} value={item.quantity} onChange={(n) => update((l) => setQuantity(l, group.id, i, n))} />}
        ...
        {group.issues.length > 0 && <div className="list-warning"><strong>{group.kind === 'bogo' ? 'BOGO is not applied yet.' : 'Review this selection.'}</strong><ul>{group.issues.map((issue) => <li key={issue}>{issue}</li>)}</ul></div>}
